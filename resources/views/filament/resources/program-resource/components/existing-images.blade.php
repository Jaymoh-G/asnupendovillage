...
@if (!$record || !$record->exists)
    <div class="py-4 text-center text-gray-500">
        Save the program first to manage images.
    </div>
@else
    @php
        ...
        <div class="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            @foreach ($images as $image)
                <div class="group relative overflow-hidden rounded-lg bg-gray-100 shadow-sm">
                    ...
                    <div class="absolute right-1 top-1 z-10" style="opacity: 1;">
...
<script>
    function removeImage(imageId) {
        if (confirm('Are you sure you want to remove this image from the program?')) {
