@php $record = $getRecord(); @endphp @if (!$record || !$record->exists)
<div class="py-4 text-center text-gray-500">
    Save the slider first to manage images.
</div>
@else @php $images = $record->images()->orderBy('created_at', 'desc')->get();
@endphp @if ($images->isEmpty())
<div class="py-4 text-center text-gray-500">No images in this slider yet.</div>
@else
<div
    class="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5"
>
    ...
    >
        ...
        <div
            class="absolute right-1 top-1 z-10 opacity-0 transition-opacity duration-200 group-hover:opacity-100"
            style="opacity: 1"
        >
            <button
                type="button"
